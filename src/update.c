...
void char_update(void)
{
	...
	for (ch = char_list; ch != NULL; ch = ch_next) {
		...
		if (ch != NULL && is_affected(ch, gsn_plague)) {
			...
			const AFFECT_DATA *plague = is_affected(ch, gsn_plague);

			if (plague == NULL) {
				REMOVE_BIT(ch->affect_bits, AFF_PLAGUE);
				return;
			}
			...
			af.evolution    = UMAX(1, plague->evolution - 1);

			for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room) {
				if (!saves_spell(af.level - 2, vch, DAM_DISEASE)
				    &&  !IS_IMMORTAL(vch)
				    &&  !is_affected(vch, gsn_plague) && number_bits(4) == 0) {
			...
			dam = UMIN(ch->level, plague->level / 5 + 1);
			ch->mana -= dam;
			ch->stam -= dam;
		...
		if (ch != NULL && is_affected(ch, gsn_poison) && !is_affected(ch, gsn_slow)) {
			const AFFECT_DATA *poison = is_affected(ch, gsn_poison);
...
void obj_update(void)
{
	...
	for (obj = object_list; obj != NULL; obj = obj_next) {
		...
		AFFECT_DATA *paf, *paf_next;
		for (paf = obj->perm_affected; paf != NULL; paf = paf_next) {
			paf_next = paf->next;

			if (paf->duration < 0)
				continue;

			obj->affects_modified = TRUE;
...
bool eligible_aggressor(CHAR_DATA *ch)
{
	return (IS_NPC(ch)
	        ...
	        && ch->fighting == NULL
	        && !is_affected(ch, gsn_calm | AFF_CHARM)
	       );
}
