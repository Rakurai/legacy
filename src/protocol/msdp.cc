...
void write_msdp_to_descriptor(DESCRIPTOR_DATA *d, const char *src, int length);
int msdp2json(const unsigned char *src, int srclen, char *out);
...
void msdp_update_all(const char *var, const char *fmt, ...)
{
...
char *msdp_get_var(DESCRIPTOR_DATA *d, const char *var)
{
	int index;
...
void process_msdp_index_val(DESCRIPTOR_DATA *d, int var_index, const char *val )
{
	int val_index;
...
void process_msdp_array( DESCRIPTOR_DATA *d, int var_index, const char *val)
{
	char buf[MAX_STRING_LENGTH];
