...
void init_mth(void)
{
	mud = (MUD_DATA *)calloc(1, sizeof(MUD_DATA));

	mud->mccp_len = COMPRESS_BUF_SIZE;
	mud->mccp_buf = (unsigned char *)calloc(COMPRESS_BUF_SIZE, sizeof(unsigned char));
...
{
	d.mth = (MTH_DATA *)calloc(1, sizeof(MTH_DATA));
...
void arachnos_devel(const char *fmt, ...)
{
	// mud specific broadcast goes here
}

void arachnos_mudlist(const char *fmt, ...)
{
